...
from BackgroundGen_par import BackgroundGen
import numpy as np
import scipy
...
class ImageGenPar:
    ...
    def __init__(self, configs):
        ...
        master_file_path = os.path.join(self.configuration['synthetic_image_directory'], self.filenames,
                                        self.configuration['master_file_name'] + self.filenames + '_' + self.configuration['sets'][0])
    ...
    def gen_images_and_file(self):
        ...
        for idx in range(start, end):
            ...
            #  open image
            background_image = back_gen.known_crop(row)
            ...
            for jdx in range(0, self.configuration['num_frames']):
                ...
                noise = np.random.normal(loc=0, scale=row['Stack_Standard_Deviation'],
                                         size=background_image.shape)

                if row['Asteroid_Present'] == False:
                    final_image = back_gen.process(background_image + noise, row)
                else:
