        ...
        if 'f' in self.options:
            ...
            for idx, col_name in enumerate(col_names[0:5]):
                ...
                if self.dist_range is None:
                    ...
                    maxi = self.dist_range[idx][1]
                    mini = self.dist_range[idx][0]

                if 'q' in self.options:
                    f = Fitter(d, distributions=get_common_distributions(), xmax=maxi, xmin=mini)
                else:
                    f = Fitter(d, xmax=maxi, xmin=mini)
                f.fit()
            ...
            for idx, row in best_dist_file.iterrows():
                dist_list.append([row['Parameter'], row['Distribution'], row['Values']])
                dist_ranges.append(row['Ranges'])
        else:
            raise ValueError("Specify file or fit in options")
